...
class PPG(AbstractActorCritic):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.aux = self.critic_network(self._critic_input_size, 1, **self._critic_network_kwargs)


        self._bm_fuse(self.actor, prefix="actor.")
        self._bm_fuse(self.critic, prefix="critic.")

        self._register_serializable("actor", "critic","aux")

        self.learning_rate = learning_rate
        self.optimizer = optim.Adam(list(self.actor.parameters())+list(self.critic.parameters()), lr=self.learning_rate)
        self.aux_optimier = optim.Adam(self.aux.parameters(), lr=self.learning_rate)

        self._register_serializable("learning_rate", "optimizer","aux_optimizer")
    ...
    def parameters(self):
        params = list(self.actor.parameters()) + list(self.critic.parameters()) + list(self.aux.parameters())
    ...
    @Benchmarkable.register
    def _compute_aux_loss(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        # ppg aux_loss
        evaluation = self.aux.forward(batch["critic_observations"])

        returns = batch["advantages"] + batch["values"]
        aux_loss = (evaluation - returns).pow(2).mean()
        return aux_loss
    ...
    def update(self, dataset: Dataset) -> Dict[str, Union[float, torch.Tensor]]:
        ...
        total_aux_loss = torch.zeros(self._batch_count)
    ...
    def _compute_value_loss(self, batch: Dict[str, torch.Tensor]) -> torch.Tensor:
        # print(f'self.recurrent: {self.recurrent}')
        # exit()
