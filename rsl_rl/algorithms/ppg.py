...
from rsl_rl.modules import Network, DoubleHeadGaussianNetwork
# from rsl_rl.storage.rollout_storage import RolloutStorage
from rsl_rl.storage.ppg_storage import PPGStorage
from rsl_rl.storage.storage import Dataset


class PPG(AbstractActorCritic):
    """Phasic Policy Gradient algorithm.

    This is an implementation of the PPG algorithm by Cobbe et. al. for vectorized environments.

    Paper: https://arxiv.org/pdf/2009.04416
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.storage = PPGStorage(self.env.num_envs, device=self.device, n_pi=n_pi)
        ...
        self.actor = DoubleHeadGaussianNetwork(
    ...
    def draw_actions(
        ...
        mean, std, _ = self.actor.forward(actor_obs, compute_std=True)
    ...
    def update(self, dataset: Dataset) -> Dict[str, Union[float, torch.Tensor]]:
        ...
        for idx, batch in enumerate(self.storage.batch_generator(self._batch_count, trajectories=self.recurrent)):
            if self.recurrent:
                ...
                action_mean, action_std, _ = self.actor.forward(observations, hidden_state=hidden_state, compute_std=True)
                ...
                action_mean, action_std, _ = self.actor.forward(batch["actor_observations"], compute_std=True)
        ...
        # Auxiliary Stage
        if self.storage.idx == 0:   # self.storage._idx == 0 means the buffer is full, which means it has been n_pi iterations since last auxiliary stage
            print("-------------------auxiliary stage--------------------")
            torch.autograd.set_detect_anomaly(True)
            batches = []
            batches_actions_dist = [] # store the policy before auxiliary stage begins
            for batch in self.storage.overall_batch_generator(self._batch_count * self._n_pi, trajectories=self.recurrent):
                batches.append(batch)
                action_mean, action_std, _ = self.actor.forward(batch["actor_observations"], compute_std=True)
                action_mean = action_mean.detach()
                action_std = action_std.detach()
                batches_actions_dist.append(torch.distributions.Normal(action_mean, action_std))
            for _ in range(self._e_aux):
                for batch_idx in range(len(batches)):
                    batch = batches[batch_idx]
                    old_actions_dist = batches_actions_dist[batch_idx]
                    if self.recurrent:
                        transition_obs = batch["actor_observations"].reshape(*batch["actor_observations"].shape[:2], -1)
                        observations, data = transitions_to_trajectories(transition_obs, batch["dones"])
                        hidden_state_h, _ = transitions_to_trajectories(batch["actor_state_h"], batch["dones"])
                        ...
                        action_mean, action_std, policy_v = self.actor.forward(observations, hidden_state=hidden_state, compute_std=True)
                        ...
                        action_mean, action_std, policy_v = self.actor.forward(batch["actor_observations"], compute_std=True)
                    actions_dist = torch.distributions.Normal(action_mean, action_std)
                    joint_loss = self._compute_joint_loss(batch, actions_dist, old_actions_dist, policy_v)
                    value_loss = self._compute_value_loss(batch)
                    
                    loss = joint_loss + value_loss
                    self.optimizer.zero_grad()
                    loss.backward()
                    nn.utils.clip_grad_norm_(self.parameters(), self._gradient_clip)
                    self.optimizer.step()
    ...
    def _compute_joint_loss(
        self, batch: Dict[str, torch.Tensor], actions_dist: torch.distributions.Normal, old_actions_dist: torch.distributions.Normal, policy_v: torch.Tensor
    ) -> torch.Tensor:
        value_clipped = batch["values"] + (policy_v - batch["values"]).clamp(-self._clip_ratio, self._clip_ratio)
        returns = batch["advantages"] + batch["values"]
        value_losses = (policy_v - returns).pow(2)
        value_losses_clipped = (value_clipped - returns).pow(2)
        aux_loss = torch.max(value_losses, value_losses_clipped).mean()
        return aux_loss + self._clone_coeff * torch.distributions.kl_divergence(old_actions_dist, actions_dist).mean()
