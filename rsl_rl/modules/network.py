...
class DoubleHeadNetwork(Benchmarkable, nn.Module):
    '''
    Design for PPG algorithm. The actor of PPG algorithm has two output heads: pi_theta & V_{theta_pi}
    '''
    recurrent_module_lstm = "LSTM"
    recurrent_module_transformer = "TF"
    ...
    def __init__(
        ...
        for i in range(len(activations) - 1):
            ...
            layers_before_last.append(layer)
            layers_before_last.append(activation)

        self._layers_before_last = nn.Sequential(*layers_before_last)
        self._pi_head = nn.Linear(dims[-1], output_size[0])
        self._pi_head_activation = get_activation(activations[-1])
        self._v_head = nn.Linear(dims[-1], output_size[1])
        self._v_head_activation = get_activation(activations[-1])

        if len(layers_before_last) > 0:
            self._init(self._layers_before_last, fade=init_fade, gain=init_gain)
        
        current_gain = init_gain / 100.0 if init_fade else init_gain
        nn.init.xavier_normal_(self._pi_head.weight, gain=current_gain)
        nn.init.xavier_normal_(self._v_head.weight, gain=current_gain)
    ...
    def forward(self, x: torch.Tensor, hidden_state=None) -> Tuple[torch.Tensor]:
        """
        Args:
            x (torch.Tensor): The input data.
            hidden_state (Tuple[torch.Tensor, torch.Tensor]): The hidden state of the network. If None, the hidden state
        ...
        # output = squeeze_preserve_batch(self._layers(input))
        output_before_last = self._layers_before_last(input)
        output_pi = squeeze_preserve_batch(self._pi_head_activation(self._pi_head(output_before_last)))
        output_v = squeeze_preserve_batch(self._v_head_activation(self._v_head(output_before_last)))

        return output_pi, output_v
